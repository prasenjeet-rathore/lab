...
Usage:
  from credit_risk.core.config import PATHS, SPLIT_DATES, LEAKAGE_COLUMNS, CONTINUOUS_FEATURES
"""
...
# config.py → core/ → credit_risk/ → src/ → project root

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent

PATHS = {
    'raw_data':           PROJECT_ROOT / 'data' / 'raw' / 'LoanData.csv',
    'data_dictionary':    PROJECT_ROOT / 'references' / 'data_dictionary.csv',
    'cleaned':            PROJECT_ROOT / 'data' / 'processed' / 'pd' / '01_estonia_cleaned.parquet',
    'target_built':       PROJECT_ROOT / 'data' / 'processed' / 'pd' / '02_estonia_target_creation.parquet',
    'final_dir':          PROJECT_ROOT / 'data' / 'processed' / 'pd' / 'final',
    'models_dir':         PROJECT_ROOT / 'models' / 'pd',           # notebooks only
    'production_models_dir': PROJECT_ROOT / 'models' / 'pd' / 'production',  # train.py → inference.py
