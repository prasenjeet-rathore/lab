...
All model-specific logic lives in credit_risk/pd/trainers/.

Artifact strategy:
  - models/pd/production/ → written by this script; read by inference.py
  - mlruns/            → MLflow experiment tracking (metrics + params committed;
                          mlruns/**/artifacts/ gitignored per .gitignore)

Usage:
    uv run python -m credit_risk.pd.train
    uv run python -m credit_risk.pd.train --model lr      # train only logistic regression
    uv run python -m credit_risk.pd.train --model xgb     # train only xgboost baseline
    uv run python -m credit_risk.pd.train --tune          # run Optuna HPO for XGBoost
    uv run python -m credit_risk.pd.train --n-trials 150  # set number of Optuna trials
"""
...
from credit_risk.pd.trainers import (
    LRTrainer,
    OptunaParams,
    BaselineParams,
    TrainingData,
...
from credit_risk.core.config import PATHS, XGB_CATEGORICAL_COLS
